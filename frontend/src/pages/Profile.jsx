...
export default function Profile() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2, ease: 'easeOut' }} className="flex h-screen bg-[#000000] text-[#E5E5E5]/90 font-inter overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-10 pb-24 md:pb-0 custom-scrollbar">
        <div className="max-w-3xl mx-auto space-y-6 md:space-y-8">
          ...
          <div>
            ...
            <p className="text-[#E5E5E5]/50 text-sm mt-1">Manage your account details and preferences.</p>
          </div>

          {/* Profile Card */}
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-[#0A0A0A] border border-[#222222] rounded-3xl p-8 shadow-xl">
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
              
              {/* Avatar */}
              <div className="relative group shrink-0">
                {avatar ? (
                  <img src={avatar} alt="avatar" className="w-24 h-24 rounded-2xl object-cover border-2 border-[#222222]" />
                ) : (
                  <div className={`w-24 h-24 rounded-2xl bg-gradient-to-br ${avatarGrad} flex items-center justify-center text-4xl font-black text-white border-2 border-[#222222]`}>
                    {initials}
                  </div>
                )}
                <button
              ...
              <div className="flex-1 text-center sm:text-left">
                {isEditing ? (
                  <div className="space-y-3">
                    <div>
                      <label className="text-[10px] text-[#E5E5E5]/40 uppercase tracking-widest font-bold block mb-1">Display Name</label>
                      <input
                        value={editName}
                        onChange={e => setEditName(e.target.value)}
                        maxLength={50}
                        className="w-full bg-[#000000] border border-[#222222] rounded-xl px-4 py-2.5 text-white text-sm outline-none focus:border-[#FFFFFF]/50 transition-colors"
                      />
                    </div>
                    <div>
                      <label className="text-[10px] text-[#E5E5E5]/40 uppercase tracking-widest font-bold block mb-1">Bio (max 200 chars)</label>
                      <textarea
                        value={editBio}
                        ...
                        className="w-full bg-[#000000] border border-[#222222] rounded-xl px-4 py-2.5 text-white text-sm outline-none focus:border-[#FFFFFF]/50 transition-colors resize-none custom-scrollbar"
                      />
                      <p className="text-[10px] text-[#E5E5E5]/30 text-right mt-0.5">{editBio.length}/200</p>
                    </div>
                    <div className="flex gap-3 pt-2">
                      <button onClick={handleSave} disabled={isSaving} className="flex-1 py-2.5 bg-[#FFFFFF] text-[#000000] text-sm font-black rounded-xl hover:opacity-90 disabled:opacity-40 transition-opacity">
                        {isSaving ? 'Saving...' : 'Save Changes'}
                      </button>
                      <button onClick={() => { setIsEditing(false); setEditName(userName); setEditBio(bio); }} className="px-5 py-2.5 bg-[#222222]/60 border border-[#222222] text-[#E5E5E5]/60 text-sm font-bold rounded-xl hover:bg-[#222222] transition-colors">
                        Cancel
                      </button>
                    </div>
                  </div>
                  ...
                  <>
                    <h3 className="text-2xl font-black text-[#E5E5E5]">{userName}</h3>
                    <p className="text-sm text-[#E5E5E5]/40 mt-0.5">{email}</p>
                    {bio && <p className="text-sm text-[#E5E5E5]/60 mt-3 leading-relaxed">{bio}</p>}
                    {!bio && <p className="text-sm text-white/25 mt-3 italic">No bio yet. Click Edit Profile to add one.</p>}
                    <button
                      onClick={() => setIsEditing(true)}
                      className="mt-4 flex items-center gap-2 text-[11px] font-bold text-[#E5E5E5]/50 hover:text-white bg-[#222222]/60 hover:bg-[#222222] px-4 py-2 rounded-xl border border-[#222222] transition-colors"
                    >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
                className="bg-[#0A0A0A] border border-[#222222] rounded-2xl p-5">
                <div className="flex items-center gap-2 mb-3">
                  <span className={`material-symbols-outlined text-[18px] ${card.color}`}>{card.icon}</span>
                  <p className="text-[10px] text-[#E5E5E5]/40 uppercase tracking-widest font-bold">{card.label}</p>
                </div>
