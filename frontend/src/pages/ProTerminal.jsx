...
export default function ProTerminal() {
  ...
  const loadAccount = useCallback(async (signal) => {
    const [userResponse, portfolioResponse, historyResponse] = await Promise.all([
      ...
      apiFetch(`${API_URL}/api/trade/history`, { signal }),
    ]);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        const response = await apiFetch(`${API_URL}/api/trade/live-prices`, { method: 'POST', body: JSON.stringify({ symbols: [symbol] }), signal: controller.signal });
