...
const ResetConfirmToast = ({ t, onConfirm }) => {
  ...
  return (
    <div className="flex flex-col gap-6 p-6 font-inter w-full max-w-[500px] border border-red-500/20 bg-[#0A0906] rounded-3xl">
      <div className="text-center">
        ...
        <p className="text-base text-[#F5F0E8]/70 mt-4 leading-relaxed px-2">
      ...
      <div className="flex gap-4 mt-6">
        <button onClick={() => toast.dismiss(t.id)} className="flex-1 py-5 rounded-2xl text-sm font-bold uppercase tracking-widest bg-[#1C1710] text-[#F5F0E8]/50 hover:text-white hover:bg-[#2A2318] transition-colors">
          Cancel
        </button>
        <button
...
const PnLTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-[#1C1710] border border-[#2A2318] rounded-2xl px-4 py-3 shadow-2xl backdrop-blur-xl">
        <p className="text-[10px] text-[#F5F0E8]/40 uppercase tracking-widest font-bold mb-1">{label}</p>
...
const UniversalTradeModal = ({ symbol, marketData, onClose, balance, token, refreshData, ownedQty = 0 }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center backdrop-blur-md z-50 p-4 font-inter">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-[#131009]/80 backdrop-blur-2xl rounded-3xl border border-[#2A2318] shadow-2xl w-full max-w-[900px] overflow-hidden flex flex-col md:flex-row max-h-[90vh]">
        <div className="bg-black/40 w-full md:w-[65%] p-6 border-b md:border-b-0 md:border-r border-[#2A2318] flex flex-col">
          <div className="flex justify-between items-start mb-6">
            <div>
              <h3 className="text-3xl font-black text-white tracking-tight">{symbol}</h3>
              <p className="text-[10px] text-[#F5F0E8]/50 uppercase tracking-widest font-bold mt-1">{INDICES.includes(symbol) ? 'MARKET INDEX' : 'EQUITY • NSE'}</p>
            </div>
          ...
          <div className="flex-1 min-h-[300px] w-full border border-[#2A2318] rounded-2xl overflow-hidden relative bg-[#0A0906] group">
            <SmartChart symbol={symbol} currentPrice={currentPrice} isGreen={isGreen} />
          </div>
          <div className="grid grid-cols-2 gap-4 mt-6">
            <div className="bg-[#1C1710] p-4 rounded-2xl border border-[#2A2318] flex justify-between items-center">
              <span className="text-[10px] uppercase text-[#F5F0E8]/50 font-bold tracking-widest">Day Low</span>
              <span className="text-sm font-mono text-[#F5F0E8]/90">₹{dayLow.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>
            </div>
            <div className="bg-[#1C1710] p-4 rounded-2xl border border-[#2A2318] flex justify-between items-center">
              <span className="text-[10px] uppercase text-[#F5F0E8]/50 font-bold tracking-widest">Day High</span>
              <span className="text-sm font-mono text-[#F5F0E8]/90">₹{dayHigh.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>
            </div>
          </div>
        </div>
        <div className="w-full md:w-[35%] p-6 flex flex-col bg-transparent">
          <div className="flex justify-end mb-6">
            <button onClick={onClose} className="text-[#F5F0E8]/40 hover:text-white bg-[#1C1710] p-1.5 rounded-xl transition-colors"><span className="material-symbols-outlined text-[20px]">close</span></button>
          </div>
          <div className="flex bg-[#1C1710] p-1.5 rounded-2xl mb-8 border border-[#2A2318]">
            <button onClick={() => setActiveTab('BUY')} className={`flex-1 py-3 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${activeTab === 'BUY' ? 'bg-green-500 text-[#003a00] shadow-md' : 'text-[#F5F0E8]/50 hover:text-white'}`}>Buy</button>
            <button onClick={() => setActiveTab('SELL')} className={`flex-1 py-3 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${activeTab === 'SELL' ? 'bg-red-500 text-white shadow-md' : 'text-[#F5F0E8]/50 hover:text-white'}`}>Sell</button>
          </div>
          <form onSubmit={handleExecute} className="flex-1 flex flex-col">
            <div className="space-y-4 flex-1">
              <div className="flex justify-between items-center bg-[#0A0906] rounded-xl px-4 py-2.5 border border-[#2A2318]">
                <span className="text-[10px] text-[#F5F0E8]/40 uppercase tracking-widest font-bold">{activeTab === 'BUY' ? 'Max Affordable' : 'Units Owned'}</span>
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  <label className="text-[#F5F0E8]/50 text-[10px] uppercase tracking-widest font-bold">Quantity (Units)</label>
                ...
                <input autoFocus type="number" value={qty} onChange={(e) => setQty(e.target.value)} className={`w-full bg-[#0A0906] border rounded-2xl p-4 text-white outline-none font-mono text-xl transition-colors ${activeTab === 'BUY' ? 'border-[#2A2318] focus:border-green-500' : 'border-[#2A2318] focus:border-red-500'}`} placeholder="0" required min="1" step="1" />
              </div>
              <div className="p-4 bg-[#1C1710] rounded-2xl border border-[#2A2318]">
                <span className="block text-[10px] uppercase text-[#F5F0E8]/50 font-bold tracking-widest mb-1">Limit Price</span>
                <span className="text-lg font-mono text-[#F5F0E8]/90">₹{currentPrice.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits:2})}</span>
              </div>
            </div>
            <div className="mt-8">
              <div className="flex justify-between items-center mb-6 px-1">
                <span className="text-[10px] font-bold text-[#F5F0E8]/50 uppercase tracking-widest">Est. Total</span>
                <span className="text-xl font-black font-mono text-[#F5F0E8]">₹{(Number(qty) * currentPrice || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits:2 })}</span>
              </div>
        ...
        <div className="max-w-7xl mx-auto space-y-6 md:space-y-8 relative z-10">
          ...
          <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 md:gap-6 mb-4 md:mb-8">
            <div>
              ...
              <p className="text-[#F5F0E8]/60 text-sm mt-1">Track your holdings, analyze P&L, and manage positions.</p>
            </div>
            <div className="text-right flex flex-col items-end gap-2">
               <div>
                 <p className="text-[10px] font-bold text-[#F5F0E8]/50 uppercase tracking-widest mb-1">Available Margin</p>
                 <p className="text-xl font-bold font-mono text-[#F5F0E8]">₹{balance.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
               </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-[#131009] border border-[#2A2318] rounded-3xl p-6 shadow-xl">
               <div className="flex items-center gap-3 mb-4 opacity-70">
                 <span className="material-symbols-outlined text-[#F5F0E8]">account_balance_wallet</span>
                 <h3 className="text-xs font-bold text-white uppercase tracking-widest">Invested Value</h3>
               </div>
               <p className="text-3xl font-bold font-mono text-[#F5F0E8]">₹{totalInvested.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
            </div>
            <div className="bg-[#131009] border border-[#2A2318] rounded-3xl p-6 shadow-xl relative overflow-hidden">
               <div className="flex items-center gap-3 mb-4 opacity-70 relative z-10">
                 <span className="material-symbols-outlined text-[#F5F0E8]">monitoring</span>
            ...
            <div className="bg-[#131009] border border-[#2A2318] rounded-3xl p-6 shadow-xl">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl bg-[#2A2318]/60 flex items-center justify-center">
                    <span className="material-symbols-outlined text-[#F5F0E8]/50 text-[20px]">area_chart</span>
                  </div>
                  <div>
                    <h3 className="font-bold text-white text-base">Profit & Loss by Position</h3>
                    <p className="text-[10px] text-[#F5F0E8]/40 uppercase tracking-widest font-semibold mt-0.5">Unrealized P&L per holding</p>
                  </div>
                </div>
              ...
              {pnlChartData.length > 0 ? (
                ...
                <div className="h-64 flex items-center justify-center text-[#F5F0E8]/30 text-sm">
                  No chart data available
                </div>
              )}

              <div className="mt-6 pt-5 border-t border-[#2A2318]">
                <p className="text-[10px] text-[#F5F0E8]/30 uppercase tracking-widest font-bold mb-3">Position Summary</p>
                  ...
                  {pnlChartData.map((d, i) => {
                    ...
                    return (
                      <div key={i} className={`rounded-2xl p-3 border ${isP ? 'bg-green-500/5 border-green-500/15' : 'bg-red-500/5 border-red-500/15'}`}>
                        <p className="text-[10px] font-bold text-[#F5F0E8]/50 uppercase tracking-widest truncate">{d.name}</p>
          ...
          <div className="md:hidden bg-[#131009] border border-[#2A2318] rounded-3xl overflow-hidden shadow-xl">
            <div className="p-4 border-b border-[#2A2318] bg-[#1C1710]/50 flex justify-between items-center">
              <h3 className="font-bold text-white text-base flex items-center gap-2">
                <span className="material-symbols-outlined text-[#F5F0E8]/50 text-[18px]">inventory_2</span> Active Positions
              </h3>
              <span className="text-[10px] font-bold text-[#F5F0E8]/40 uppercase tracking-widest bg-[#2A2318]/60 px-3 py-1 rounded-lg">{holdings.length} Assets</span>
            </div>
            {isLoading ? (
              ...
              <div className="p-10 text-center">
                <span className="material-symbols-outlined text-[#F5F0E8]/20 text-4xl">inbox</span>
                <p className="text-[#F5F0E8]/40 font-medium mt-3">Portfolio is empty.</p>
              ...
              <div className="divide-y divide-[#2A2318]">
                {holdings.map((pos, i) => {
                  ...
                  const isProfit = pnl >= 0;
                  return (
                    <div key={i} onClick={() => setSelectedAsset(pos.symbol)} className="p-4 flex items-center justify-between hover:bg-white/[0.02] active:bg-[#2A2318]/60 transition-colors cursor-pointer">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-[#1C1710] border border-[#2A2318] flex items-center justify-center font-bold text-xs text-[#F5F0E8]/80">
                          {pos.symbol.charAt(0)}
                        </div>
                        <div>
                          <p className="font-bold text-[#F5F0E8]/90 text-sm">{pos.symbol}</p>
                          <p className="text-[10px] text-[#F5F0E8]/40 mt-0.5">{pos.quantity} units · avg ₹{pos.avgPrice.toLocaleString('en-IN', {minimumFractionDigits: 0, maximumFractionDigits: 0})}</p>
                        </div>
                      </div>
                      <div className="text-right">
          ...
          <div className="hidden md:block bg-[#131009] border border-[#2A2318] rounded-3xl overflow-hidden shadow-xl">
             <div className="p-6 border-b border-[#2A2318] bg-[#1C1710]/50 flex justify-between items-center">
               <h3 className="font-bold text-white text-lg flex items-center gap-2">
                 <span className="material-symbols-outlined text-[#F5F0E8]/50 text-[20px]">inventory_2</span> Active Positions
               </h3>
               <span className="text-[10px] font-bold text-[#F5F0E8]/40 uppercase tracking-widest bg-[#2A2318]/60 px-3 py-1 rounded-lg">
                     ...
                     <tr>
                       <td colSpan="7" className="px-8 py-16 text-center">
                         <div className="flex flex-col items-center gap-4">
                           <div className="w-16 h-16 rounded-full bg-[#2A2318]/60 flex items-center justify-center">
                             <span className="material-symbols-outlined text-[#F5F0E8]/20 text-3xl">inbox</span>
                           </div>
                           <p className="text-[#F5F0E8]/40 font-medium">Your portfolio is empty.</p>
                         ...
                         <tr key={i} className="hover:bg-white/[0.02] transition-colors group cursor-pointer" onClick={() => setSelectedAsset(pos.symbol)}>
                           <td className="px-8 py-4">
                             <div className="flex items-center gap-4">
                               <div className="w-10 h-10 rounded-xl bg-[#1C1710] border border-[#2A2318] flex items-center justify-center font-bold text-xs text-[#F5F0E8]/80 group-hover:bg-[#2A2318] transition-colors">
                                 {pos.symbol.charAt(0)}
                               </div>
                               <div>
                                 <p className="font-bold text-[#F5F0E8]/90 group-hover:text-white transition-colors">{pos.symbol}</p>
                           ...
                           <td className="px-8 py-4 text-right">
                              <p className="font-mono text-[#F5F0E8]/70">₹{pos.avgPrice.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
                           </td>
                           <td className="px-8 py-4 text-right">
                           ...
                           <td className="px-8 py-4 text-center">
                              <button 
                                ...
                                className="bg-[#1C1710] border border-[#2A2318] hover:border-white/20 text-[#F5F0E8]/80 hover:text-white px-4 py-2 rounded-xl text-[11px] font-bold uppercase tracking-wider transition-colors"
                              >
                                Exit / Add
                              </button>
                           </td>
